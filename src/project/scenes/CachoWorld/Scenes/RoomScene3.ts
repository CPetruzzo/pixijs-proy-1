...
export class RoomScene3 extends PixiScene {
	...
	constructor(playerId: string, roomId: string) {
		...
		console.log(`Room3Scene (Office) created for player ${playerId} in room ${roomId}`);

		this.worldContainer.name = "WorldContainer";
		this.addChild(this.worldContainer);

		// Crear mapa con textura office
		this.createMap();
		...
		console.log("Room3Scene setup complete");
	}

	private createMap(): void {
		this.worldMap = new Container();
		const backgroundSprite = Sprite.from("office");
	...
	public override destroy(_options?: any): void {
		console.log(`Destroying Room3Scene for room ${this.roomId}`);
		...
		console.log(`Room3Scene ${this.roomId} destroyed successfully`);
	}
}
