...
import { Container, Graphics, Text, BlurFilter, Sprite, Ticker } from "pixi.js";
...
export class GameplayScene extends PixiScene {
	...
	private selectedFireNode: any | null = null; // Cambiado a any para facilitar acceso a propiedades personalizadas
	...
	private evidenceText: Text; // Nueva propiedad
	constructor() {
		super();
		...
		// Iniciamos con el tutorial previo
		this.showHowToPlay();
	...
	private showHowToPlay(): void {
		...
		const title = new Text("INSTRUCCIONES", {
			fill: 0xffaa00,
			fontSize: 50,
			fontWeight: "900",
		});
		...
		const instructions = [
			"🔥 El fuego avanza cada 3 segundos.",
			"🌲 Si llegas a 100.000 Ha perdidas, el juego termina.",
			...
			"Las manifestaciones son mejores cuando más gente está al tanto",
	...
	private restartGame(): void {
		...
		// 3. Re-inicializar escena
		this.setupBoard();
		this.setupPlayers();
		this.setupUI();
		this.evidencePoints = 0;
		this.updateEvidenceUI(); // <--- Resetear UI
		// 4. Volver a mostrar selección o tutorial
	...
	private createFireNode(data: any): Container {
		...
		container.on("pointertap", () => {
			...
			// Seguridad: Resetear escala del anterior solo si existe y no ha sido destruido
	...
	private reduceFire(node: any, amount: number): void {
		...
		if (node.intensity <= 0) {
			// Si el nodo destruido es el seleccionado, limpiamos la referencia
	...
	private playCard(card: any): void {
		...
		// Seguridad: Solo usar cartas de ataque si hay un foco seleccionado y VIVO
		if (type === "BOMBEROS" || type === "HIDROAVION") {
		...
		} else if (type === "DENUNCIA" || type === "PEDIDO_INFORME") {
			this.evidencePoints++;
			if (type === "PEDIDO_INFORME") {
				this.visibilityScore += 5;
			}
			this.updateEvidenceUI(); // <--- Llamada aquí
			used = true;
		} else if (type === "POST_VIRAL" || type === "VIDEO_DRONE") {
			this.visibilityScore += type === "POST_VIRAL" ? 15 : 10;
			used = true;
			...
			this.hectareasQuemadas = Math.max(0, this.hectareasQuemadas - 1500);
			used = true;
		}
	...
	private async endTurn(): Promise<void> {
		...
		// Reset de selección al terminar turno
		if (this.selectedFireNode && !this.selectedFireNode.destroyed) {
		...
		if (this.playerRole !== "POLITICO_BUENO" && this.isPoliticoBuenoPresent) {
			...
			if (Math.random() > 0.5) {
				this.evidencePoints++;
				this.updateEvidenceUI(); // <--- Llamada aquí también por si el rol actual lo requiere
			} else {
				this.isPoliticoMaloBlocked = true;
			}
			await new Promise((r) => setTimeout(r, 1000));
	...
	// --- RESTO DE MÉTODOS (UI, ASSETS, ETC) ---
	...
	private win(): void {
		...
		reflection.forEach((line, i) => {
			setTimeout(() => {
				...
				// Si es el último mensaje, mostramos el botón de reinicio
	...
	private calculateMaxCards(): number {
		// Si no es Brigadista, no tiene estas restricciones
		if (this.playerRole !== "BRIGADISTA") {
			return 5;
		}

		// Umbrales de Brigadista:
		// 1. Más de 60% visibilidad -> Todas las cartas (5)
		if (this.visibilityScore > 60) {
			return 5;
		}

		// 2. 50% visibilidad o 75.000 Ha -> 4 cartas
		if (this.visibilityScore >= 50 || this.hectareasQuemadas >= 75000) {
			return 4;
		}

		// 3. 25% visibilidad o 65.000 Ha -> 3 cartas
		if (this.visibilityScore >= 25 || this.hectareasQuemadas >= 65000) {
			return 3;
		}

		// 4. Estado inicial -> 2 cartas
		return 2;
	}
	...
	private gameOver(): void {
		...
		// Mostramos el botón tras un breve delay para que vean el mensaje de derrota
		setTimeout(() => this.showRestartButton(), 2000);
	}
	...
	private setupUI(): void {
		...
		// Nuevo contador de pruebas para el político
		this.evidenceText = new Text("", { fill: 0xffffff, fontSize: 22, fontWeight: "bold", stroke: 0x000000, strokeThickness: 4 });
		this.evidenceText.position.set(495, -610); // Ubicado debajo de Conciencia
		this.evidenceText.visible = false; // Oculto por defecto
		...
		endTurnBtn.addChild(finalizar);
		endTurnBtn.position.set(0, 400);
		endTurnBtn.eventMode = "static";
		endTurnBtn.cursor = "pointer";
		endTurnBtn.on("pointertap", () => {
	...
	private showRoleSelection(): void {
		...
		const roles: { id: PlayerRole; title: string; desc: string; color: number }[] = [
			...
			{ id: "CIUDADANO", title: "LA ASAMBLEA", desc: "Objetivo: 100% Conciencia Social.\nAliados: Todos luchan juntos.", color: 0x2ecc71 },
		];
	...
	private setupPlayers(): void {
		...
		pData.forEach((p) => {
			...
			// const b = new Graphics().beginFill(0x111111).lineStyle(2, p.color).drawRoundedRect(-120, -40, 240, 80, 10).endFill();
			// const t = new Text(p.name, { fill: p.color, fontSize: 14, fontWeight: "bold" });
