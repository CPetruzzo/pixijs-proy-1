...
export class Ingredient extends Sprite {
	...
	private temperature = 4;
	private requiresRefrigeration: boolean;
	private outOfFridgeTime = 0;
	private isSpoiled = false;
	private tempIndicator: Text;
	private warningIcon: Graphics;

	private readonly MAX_OUT_TIME = 15;
	private readonly DANGER_TEMP = 20;

	constructor(public key: string, x: number, y: number) {
		// CAMBIO AQUÍ: Usamos el ModManager para obtener la textura dinámica
		super(ModManager.instance.getTexture(key));
		...
		// El refrigerado se mantiene por lógica base, pero podría venir del mod también
		this.requiresRefrigeration = ["jamon", "queso", "leche", "ojo"].includes(key);
	...
	private updateTempIndicator(): void {
		...
		if (this.temperature <= 4) {
			this.tempIndicator.style.fill = "#00AAFF";
		} else if (this.temperature <= this.DANGER_TEMP) {
			this.tempIndicator.style.fill = "#FFAA00";
		} else {
			this.tempIndicator.style.fill = "#FF0000";
		}
	}
