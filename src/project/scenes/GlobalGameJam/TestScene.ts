...
export class TestScene extends PixiScene {
	...
	private gravity = 0.5; // Gravedad que afecta a la burbuja
	private lift = -8; // Fuerza hacia arriba cuando el jugador presiona espacio
	private bubbleSize = 1; // Tamaño inicial de la burbuja
	...
	constructor() {
		...
		const kitchenBackground = new TilingSprite(Texture.from("basquetcourtFullScreen"), ScaleHelper.IDEAL_WIDTH * 100, ScaleHelper.IDEAL_HEIGHT * 100);
		...
		setInterval(() => this.spawnObstacle(), 2000); // Obstáculos cada 2 segundos
		setInterval(() => this.spawnCollectibleBubble(), 1500); // Burbujas cada 1.5 segundos
	}

	private createBubble(): void {
		this.bubble = Sprite.from(Texture.from("star")); // Imagen de burbuja
		this.bubble.anchor.set(0.5);
		this.bubble.x = 200; // Posición inicial
		this.bubble.y = ScaleHelper.IDEAL_HEIGHT / 2;
		this.bubble.alpha = 0.8; // Transparencia
		this.bubble.scale.set(this.bubbleSize); // Escala inicial
		this.sceneContainer.addChild(this.bubble);
	}

	private spawnObstacle(): void {
		const obstacle = Sprite.from(Texture.from("cachoknob"));
