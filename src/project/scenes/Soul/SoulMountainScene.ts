...
import { DruidHouseScene } from "./DruidHouseScene";
...
export class SoulMountainScene extends PixiScene {
	...
	private createScene(): void {
		if (this.DEBUG_MODE) {
			this.startOptions.skipIntro = true;
		}
		...
		// 2. Lógica de Posición Inicial:
		if (this.startOptions.spawnPosition) {
			// Si venimos de la casa del druida, usamos esa posición
			this.player.x = this.startOptions.spawnPosition.x;
			this.player.y = this.startOptions.spawnPosition.y;
		} else {
			// Posición por defecto (inicio del juego)
	...
	private createInteractionPoints(): void {
		...
		this.addInteractable(
			...
			(target) => {
				...
				Manager.changeScene(DruidHouseScene);
			}
		);
		// hill prev
		this.addInteractable(
