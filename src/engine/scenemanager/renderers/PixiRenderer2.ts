...
export class PixiRenderer implements IRenderer {
	...
	public constructor(pixiOptions?: PixiRendererOptions, letterboxScale?: boolean) {
		...
		// PARCHE CRÍTICO: Monkey-patch del renderer para forzar UNSIGNED_SHORT en índices
		// Este parche intercepta el error en su origen en PixiRenderer.ts:127
		const renderer: any = this.pixiRenderer;

		// Intentar parchear el geometry renderer que está causando el error
		if (renderer.geometry) {
			const originalBind = renderer.geometry.bind;
			if (originalBind) {
				renderer.geometry.bind = function (geometry: any, shader: any) {
					// Forzar el tipo de índice ANTES de hacer bind
					if (geometry?.indexBuffer) {
						const indexBuffer = geometry.indexBuffer;
						indexBuffer.type = 5123;
						if (indexBuffer._type !== undefined) {
							indexBuffer._type = 5123;
						}
						if (indexBuffer.data instanceof Uint16Array) {
							Object.defineProperty(indexBuffer, "type", {
								get() {
									return 5123;
								},
								set() {
					...
					if (geometry?.indices?.buffer) {
						geometry.indices.buffer.type = 5123;
						if (geometry.indices.buffer._type !== undefined) {
							geometry.indices.buffer._type = 5123;
						}
					}
					return originalBind.call(this, geometry, shader);
				};
				console.log("✓ Renderer geometry.bind patched");
			}
		}

		// También parchear el buffer renderer
		if (renderer.buffer) {
			const originalUpdate = renderer.buffer.update;
			if (originalUpdate) {
				renderer.buffer.update = function (buffer: any) {
					if (buffer && buffer.data instanceof Uint16Array) {
						buffer.type = 5123;
						if (buffer._type !== undefined) {
							buffer._type = 5123;
						}
					}
					return originalUpdate.call(this, buffer);
				};
				console.log("✓ Renderer buffer.update patched");
			}
		}
